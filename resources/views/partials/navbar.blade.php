<nav class="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 p-4 shadow-lg">
    <div class="max-w-screen-xl mx-auto flex flex-col lg:flex-row items-center justify-between">
        <!-- Logo -->
        <a href="{{ route('index') }}"
        class="text-5xl font-bold text-white hover:text-orange-500 transition-transform transform hover:rotate-6 hover:scale-105 tracking-wider mb-4 lg:mb-0">
        ZMA
    </a>
        ...
        <div class="flex items-center space-x-8 text-white justify-center w-full lg:w-auto">
            ...
            @else
                <a href="{{ route('login') }}?backTo={{ url()->current() }}"
                ...
                <a href="{{ route('register') }}?backTo={{ url()->current() }}"
